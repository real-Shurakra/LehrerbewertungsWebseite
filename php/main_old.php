...
class FragenVerwaltung {
    ...
    {
        try {
            if (count("SELECT id FROM fragen WHERE frage = '" . $_REQUEST['frage'] . "' AND lehrerid = (SELECT id FROM lehrer WHERE mail = 'temp.dump@hotmail.com');") != 0)
            {
				global $link;
				
				$sqlstring_AddFrage = "INSERT INTO fragen (id, frage, kategorie, lehrerid) VALUES (26,'Test.','Unterricht',1)";//," . $_REQUEST['frage'] . "," . $_REQUEST['kategorie'] . ",1;";
                $sqlstring_AddFrage = mysqli_real_escape_string( $link, $sqlstring_AddFrage );
                //main::sendSQL($sqlstring_AddFrage);
				
				mysqli_query($link, $sqlstring_AddFrage);
